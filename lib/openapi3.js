...
function makeSchemaLink(schemaName, generics) {
    const spl = schemaName.split('_')
     if (generics && generics.length && generics.length > 0 && spl.length > 1) {
        const gen = spl[0];
         if (generics.indexOf(gen) > -1) {
            const remaining = spl.slice(1).join('');
            const genLink = makeSchemaLink(gen);
            const schemaLink = makeSchemaLink(remaining, generics);
            return genLink + '\[' + schemaLink + '\]';
        }
    }
    return '['+schemaName+'](#schema'+schemaName.toLowerCase()+')';
}
    ...
    for (let param of data.parameters) {
        ...
        if (pSchema && !param.safeType) {
            ...
            if (pSchema["x-widdershins-oldRef"]) {
                ...
                param.safeType = makeSchemaLink(schemaName, data.options.generics);
            }
            if (param.refName) param.safeType = makeSchemaLink(param.refName, data.options.generics);
        }
        if (pSchema) {
    ...
    for (let r in data.operation.responses) {
        ...
        for (let ct in response.content) {
            ...
            if (contentType.schema && contentType.schema["x-widdershins-oldRef"] && contentType.schema["x-widdershins-oldRef"].startsWith('#/components/')) {
                ...
                entry.schema = makeSchemaLink(schemaName, data.options.generics);
                entry.$ref = true;
            }
            else {
                if (
                    data.options.noInlineCollections &&
                    contentType.schema && contentType.schema.type && (contentType.schema.type === 'array') &&
                    contentType.schema.items && contentType.schema.items["x-widdershins-oldRef"] && contentType.schema.items["x-widdershins-oldRef"].startsWith('#/components/')
                ) {
